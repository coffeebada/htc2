...
const BOX_STYLE = {
  container: { 
    ...
    width: '100vw', 
    // 💡 [세로 스크롤 패치] 고정 height를 지우고 최소 높이를 주어 아래쪽 대시보드가 잘리지 않게 만듭니다.
    minHeight: '100vh', 
    color: 'white', 
    display: 'flex', 
    flexDirection: 'column', 
    alignItems: 'center', 
    justifyContent: 'flex-start', // 💡 컨텐츠들이 상단부터 차례대로 스크롤 흐름을 타도록 정렬 변경
    fontFamily: 'sans-serif', 
    boxSizing: 'border-box', 
    
    // 💡 위아래 터치 스크롤 스크롤바와 제어권을 정상 작동하도록 개방합니다.
    overflowX: 'hidden',    
    overflowY: 'auto',     
    touchAction: 'pan-y',   
    userSelect: 'none',     
    ...
    position: 'relative',
    padding: '40px 20px' // 💡 위아래 여백을 넉넉히 주어 메뉴가 잘리지 않게 방어
  },
  contentWrapper: { 
    width: '100%', 
    maxWidth: '1080px', 
...
const CIRCLE_STYLE = {
  container: { 
    ...
    // 💡 [5도권 화면 밀림 방지 핵심 패치]
    overflow: 'hidden',     // 외곽 여백 드래그 시 밀림 차단
    touchAction: 'none',    // 휠 드래그 액션 외에 화면 전체가 움직이는 것을 방지합니다.
    userSelect: 'none'
  },
  ...
  return (
    <div style={CIRCLE_STYLE.container}>
      <div style={{ position: 'absolute', top: '30px', left: '420px', zIndex: 5000 }}>
        <button onClick={onRouteClick} style={BOX_STYLE.settingsBtn}> Close & Return to DHTC </button>
      </div>
  ...
  {item.displaySig}
        ...
        >
          <div style={CIRCLE_STYLE.coreCenterContent}>
            <span style={{ fontSize: 'calc(14px + 0.6vmin)', fontWeight: 'bold', marginBottom: '4px', color: displayMode === 'harmonica' ? '#ef4444' : '#00a8ff', whiteSpace: 'nowrap' }}>
              {displayMode === 'harmonica' ? 'Harp Key' : 'Song Key'}
            </span>
            
            {/* 💡 [교체 완료] 글자 크기는 요청하신 큰 크기 그대로 유지하되, 송키 선택 시 상하 두 줄로 깔끔하게 쪼개어 출력합니다. */}
            <span style={{ 
              fontWeight: '900', 
              fontSize: 'calc(28px + 1.2vmin)', 
              color: '#3b82f6', 
              whiteSpace: 'nowrap',
              lineHeight: '1.1', // 두 줄로 나뉘었을 때 글자가 서로 붙거나 너무 멀어지지 않게 조정
              display: 'block',
              textAlign: 'center'
            }}>
