  ...
  return (
    <div style={BOX_STYLE.container}>
      <div style={BOX_STYLE.contentWrapper}>
          ...
          <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
            ...
            >
              <Settings size={22} />
                ...
                {/* 🎯 [1/3 파트: 하모니카 격자판 가로폭 메뉴 동기화 및 세로 간격 일정화 정밀 패치] */}
        {/* 상/하단 메뉴 및 믹서 패널 폭과 자석처럼 1:1 일치하도록 maxWidth: '800px' 축을 완벽히 선언했습니다. */}
        <div style={{ 
          display: 'flex', 
          gap: '10px', 
          padding: '4px 0', 
          width: '100%', 
          maxWidth: '800px',          // 👈 상단 툴바 및 하단 믹서 맥스 폭과 100% 동기화
          ...
          {HARP_LAYOUT.holes.map((h, i) => {
            return (
              /* 💡 [세로 간격 일정 정합] 각 열 내부의 자식 박스들이 단 1픽셀의 오차도 없이 일정한 세로 여백을 고수하도록 gap을 '8px'로 일괄 통일했습니다. */
              <div key={i} style={{ ...BOX_STYLE.column, display: 'flex', flexDirection: 'column', gap: '8px', margin: 0, padding: 0 }}>
                
                {/* 1. 상단 오버 존 스택 구역 (25% 슬림 두께 5.55vw 고수) */}
                <div style={{ 
                  display: 'flex', 
                  flexDirection: 'column-reverse', 
                  gap: '8px', 
                  height: 'calc(16.65vw + 16px)', 
                  maxHeight: '196px', 
                  width: '100%', 
                  ...
                  {ACTIVE_LAYOUT.topSpecials[i].map((semiVal, tIdx) => (
                    <div key={tIdx} style={{ width: '100%', height: '5.55vw', maxHeight: '60px', visibility: semiVal === null ? 'hidden' : 'visible', flexShrink: 0 }}>
                ...
                {/* 2. 표준 BLOW 행 구역 (💡 1~6번 홀 영역 위로 1줄 무배경 순수 텍스트 오버레이 매립 동기화 완결) */}
                <div style={{ width: '100%', height: '5.55vw', maxHeight: '60px', flexShrink: 0, position: 'relative' }}>
                  <NoteBox semi={ACTIVE_LAYOUT.blow[i]} getNote={getNoteName} activeNote={activeNote} cents={centsOff} limit={tolerance} onStart={handleNoteStart} onStop={handleNoteStop} showOverbanding={showOverbanding} selectedTuning={selectedTuning} />
                  ...
                  {i === 0 && (
                    <div style={{
                      ...
                      gap: '14px',                
                      position: 'absolute',
                      top: '-12px',               // 박스 높이가 5.55vw로 축소된 비례에 맞춘 정밀 수직 사잇길 틈새 정렬
                      left: '4px',                
                      zIndex: 300,                
                      ...
                      <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'nowrap', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
                        <span style={{ fontSize: '15px', fontWeight: '950', color: '#60a5fa', letterSpacing: '-0.3px', whiteSpace: 'nowrap', textShadow: '0 2px 4px rgba(0,0,0,0.9)' }}>
                          {scaleRootKey} {selectedScale}
                        </span>
                        <span style={{ fontSize: '10px', color: '#475569', fontWeight: '900', letterSpacing: '0.5px', whiteSpace: 'nowrap' }}>
                          SCALE
                        </span>
                      </div>
                      {/* 순수 텍스트형 음이름 목록 */}
                ...
                {/* 3. 중앙 가이드 홀 번호 행 */}
                <div style={{ ...BOX_STYLE.holeNumber, margin: 0, padding: 0 }}>
                  {h}
                </div>

                {/* 4. 표준 DRAW 행 구역 (25% 슬림 두께 5.55vw 고수) */}
                <div style={{ width: '100%', height: '5.55vw', maxHeight: '60px', flexShrink: 0 }}>
                  <NoteBox semi={ACTIVE_LAYOUT.draw[i]} getNote={getNoteName} activeNote={activeNote} cents={centsOff} limit={tolerance} onStart={handleNoteStart} onStop={handleNoteStop} showOverbanding={showOverbanding} selectedTuning={selectedTuning} />
                </div>

                                {/* 5. 하단 벤딩 존 스택 구역 (25% 슬림 두께 5.55vw 고수) */}
                <div style={{ 
                  display: 'flex', 
                  flexDirection: 'column', 
                  gap: '8px', 
                  ...
                  {ACTIVE_LAYOUT.bottomSpecials[i].map((semiVal, bIdx) => (
                    <div key={bIdx} style={{ width: '100%', height: '5.55vw', maxHeight: '60px', visibility: semiVal === null ? 'hidden' : 'visible', flexShrink: 0 }}>
                      <NoteBox semi={semiVal} getNote={getNoteName} activeNote={activeNote} cents={centsOff} limit={tolerance} onStart={handleNoteStart} onStop={handleNoteStop} isDrawZone={true} holeNum={h} showOverbanding={showOverbanding} selectedTuning={selectedTuning} />
                    </div>
                  ))}
                </div>

                {/* 🎯 [오피셜 타이틀 및 개발 저작권 크레딧 우측 아래 칼정렬 부활 패치 완료] */}
                {/* 음정 사각형이 25% 슬림해진 비례에 맞춰 bottom 오프셋 상수를 -38px로 정밀 리밸런싱하여 믹서 뒤로 파묻히는 현상을 완벽히 차단했습니다. */}
                {h === 10 && (
                  <div style={{ position: 'absolute', bottom: '-38px', right: '0px', width: '60vw', maxWidth: '650px', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', pointerEvents: 'none', zIndex: 10, fontFamily: 'sans-serif', lineHeight: '1.3' }}>
        ...
        </div> {/* ⏪ 1/3 파트 마감: 하모니카 격자판 컨테이너 대봉쇄 단락 */}

 
             {/* 오디오 대시보드 구역 */}
        {/* 🎯 [2/3 파트: 하단부 오디오 대시보드 맥스폭 800px 수평 가로 3등분 균등 정렬 패치] */}
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%', maxWidth: '800px', gap: '14px', boxSizing: 'border-box', marginTop: '20px' }}>
          
          {/* 🎼 [1층 축: 배킹트랙 및 정밀 속도/피치 버튼 3등분 가로 일렬 라인] */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', width: '100%', gap: '14px', alignItems: 'center', boxSizing: 'border-box' }}>
            ...
            {/* 1-2. SPEED 증감 버튼 컨트롤러 (무테두리 패치 가동) */}
            ...
            {/* 1-3. PITCH 증감 버튼 컨트롤러 (무테두리 패치 가동) */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#111827', padding: '6px 14px', borderRadius: '12px', border: '1px solid #1e293b', height: '48px', boxSizing: 'border-box' }}>
          ...
          {/* 🔴 [2층 축: 배킹트랙 아래 줄 - 믹스&레코드 기능 버튼 및 통합 볼륨 슬라이더 3등분 라인] */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', width: '100%', gap: '14px', alignItems: 'center', boxSizing: 'border-box' }}>
            
            {/* 2-1. 믹스&레코딩 기능 버튼 그룹 (레코드, 스톱앤플레이, 다운로드 3버튼 컴팩트 정렬) */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', height: '48px', boxSizing: 'border-box' }}>
              ...
              <button 
                ...
                style={{
                  flex: 1.2, height: '100%', borderRadius: '10px', border: 'none',
                  backgroundColor: isRecordedPlaying ? '#10b981' : '#111827', color: 'white',
                  fontWeight: '900', fontSize: '11px', cursor: recordedUrl ? 'pointer' : 'not-allowed', opacity: recordedUrl ? 1 : 0.4, border: '1px solid #1e293b'
                }}
              >
                {isRecordedPlaying ? 'STOP REC' : 'PLAY REC'}
              </button>
              ...
              <button 
                ...
                disabled={!recordedUrl}
                style={{
                  flex: 1, height: '100%', borderRadius: '10px', border: 'none',
            ...
            {/* 2-2. 마이크 볼륨 제어 슬라이더 박스 (하울링 및 오타 완치 필터 이식) */}
        ...
        </div> {/* ⏪ 2/3 파트 마감: 오디오 대시보드 바깥 상자 마감 */}
