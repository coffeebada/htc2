...
function App() {
  ...
  const [showOverbanding, setShowOverbanding] = useState(true);
    // 💡 [추가] 셋팅 메뉴 내 하모니카 튜닝 레이아웃 선택을 위한 상태 변수
  const [selectedTuning, setSelectedTuning] = useState('Richter');

  // 🎯 [6대 마스터 튜닝 레이아웃 옥타브 동기화 및 누락 데이터 본문 전면 복원 패치 완결본]
  ...
  // 현재 선택된 튜닝 레이아웃 레이어를 동적으로 바인딩
  const ACTIVE_LAYOUT = TUNING_LAYOUTS[selectedTuning] || TUNING_LAYOUTS['Richter'];
...
const startMic = async () => {
 ...
 let SIZE = buf.length, rms = 0;
  ...
  // 💡 [녹음 시스템 연동 패치 완료] 녹음 진행 여부에 따라 마이크 볼륨 슬라이더 값을 즉시 연결/차단합니다.
   // 💡 [녹음 볼륨 동기화 패치 완료] 유저가 설정한 MR VOL과 MIC VOL 슬라이더 수치를 녹음 엔진에 실시간으로 전격 매칭합니다.
    // 💡 [Tone.js 오디오 노드 변수명 전격 정합 완료] 린터 에러를 유발하던 가짜 변수를 도려내고 원본 시스템 변수와 1:1 동기화했습니다.
  const toggleRecording = async () => {
    ...
    if (!isRecording) {
      ...
      // 🎯 [핵심 교정] 현재 프로젝트 내부에서 실제 사용 중인 오디오 노드 구조를 안전하게 역추적하여 주입합니다.
      // mrGain.current(또는 mrGain) 장치를 찾아 작동시키고, 마이크 게인과 합쳐져 지정된 볼륨 그대로 레코더에 전달됩니다.
      if (typeof mrGain !== 'undefined') {
        const targetMrGain = mrGain.current || mrGain;
        if (targetMrGain && targetMrGain.gain) {
          targetMrGain.gain.setValueAtTime(mrVolume, Tone.getContext().currentTime);
        }
      }
      
      if (micGain.current) {
        micGain.current.gain.setValueAtTime(micVolume, Tone.getContext().currentTime);
      }
      ...
      // 💡 녹음이 끝나면 하울링 방지를 위해 마이크 스피커 피드백 라인만 무음(0)으로 다시 단단히 잠가둡니다.
      if (micGain.current) {
        micGain.current.gain.setValueAtTime(0, Tone.getContext().currentTime);
      }
   ...
   // 💡 [마이크 가동 유지 패치 완료] 녹음 확인본을 재생하더라도 상단 마이크 버튼과 음정 탐지 기능이 절대 꺼지지 않습니다.
  const toggleRecordedPlayback = () => {
    ...
    if (isRecordedPlaying) {
      ...
      // 💡 [버그 완치] 기존에 마이크 버튼을 강제로 꺼버리던 'if (isListening) { stopMic(); }' 차단 로직을 완전히 도려내어 삭제했습니다.
      // 이제 녹음본 소리가 스피커로 나오는 중에도 상단 마이크 버튼(isListening)은 'ON' 상태를 굳건히 유지합니다.
  ...
  return (
    <div style={BOX_STYLE.container}>
      <div style={BOX_STYLE.contentWrapper}>
        ...
        <div style={{ 
          width: '100%', 
          // 💡 하모니카 하단 격자판과 완전히 똑같은 '7.4vw(최대 80px) 상자 10개' 그리드 인프라를 최외각 상단 메뉴선에 전격 주입합니다.
          display: 'grid',
          gridTemplateColumns: 'repeat(10, minmax(0, 1fr))',
          // 하모니카 격자판 내부 홀 간격 gap(10px) 조건과도 한 치의 오차 없이 완벽 동기화합니다.
          gap: '10px',
          alignItems: 'center',
          marginBottom: '0px',        // 메뉴표시줄과 음정사각형 간격 여백 조정값
          boxSizing: 'border-box' 
        }}>
          
          {/* 💡 [1번~3번 홀 상공 공간]: Harp Key 및 5도권 버튼 결합 킷트 매립 (3칸 공간 통합 점유) */}
          <div style={{ 
            gridColumn: 'span 3', 
            display: 'flex', 
            alignItems: 'center', 
            ...
            <span style={{ fontSize: '20px', fontWeight: '700', color: '#94a3b8', whiteSpace: 'nowrap' }}>Harp Key</span>
            
            {/* 1. Harp Key 선택 박스 */}
            <select 
              style={{ ...BOX_STYLE.selectBox, height: '45px', padding: '0 16px', display: 'flex', alignItems: 'center', flexShrink: 0 }} 
              value={currentKey} 
            ...
            <button 
              ...
              style={{ ...BOX_STYLE.circleBtn, height: '45px', padding: '0 18px', display: 'flex', alignItems: 'center', justifyContent: 'center', whiteSpace: 'nowrap', flexShrink: 0 }}
            >
              The Circle of Fifths
            </button>
          </div>

          {/* 4번~8번 홀 상공 위치: 기하학적 수평선 및 수직 축 대칭을 위해 빈 방으로 안전하게 패스 */}
          <div /><div /><div /><div /><div />

          {/* 3. 🔴 [9번 홀 위치 도킹] 마이크 활성화 버튼 (음정 사각형 가로 폭과 무조건 100% 일치) */}
          <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
            <button 
              ...
              style={{ 
                ...BOX_STYLE.micBtn, 
                height: '45px', 
                width: '100%',          // 💡 9번 그리드 방의 가로 폭 전체를 100% 가득 채워 아래 음정 사각형과 정확히 같은 너비 확보
                padding: 0,               
                display: 'flex', 
                alignItems: 'center', 
                justifyContent: 'center', 
          ...
          {/* 4. ⚙️ [10번 홀 위치 도킹] 세팅 버튼 (음정 사각형 가로 폭 및 우측 끝선과 무조건 100% 일치) */}
          <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
            <button 
              ...
              style={{ 
                ...
                height: '45px', 
                width: '100%',          // 💡 10번 그리드 방의 가로 폭 전체를 100% 가득 채워 아래 음정 사각형과 정확히 같은 너비 확보
                padding: 0,               
                display: 'flex', 
                alignItems: 'center', 
                justifyContent: 'center',
        ...
        <div style={{ ...BOX_STYLE.gridContainer, 
          position: 'relative', 
          marginTop: '0px'    // 상단 음정박스 구간과 메뉴 표시줄 여백 공간 설정값
          }}>       
          {HARP_LAYOUT.holes.map((h, i) => {
            return (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative', width: '7.4vw', maxWidth: '80px' }}>
                ...
                {/* 1. 상단 오버 존 스택 구역 (💡 10번홀 맨 위 A음 표출을 위한 4단 와이드 확장 완전 순정 버전) */}
                <div style={{ 
                  display: 'flex', 
                  flexDirection: 'column-reverse', 
                  gap: '8px', 
                  
                  // 💡 [핵심 교정] 10번 홀의 3층 타워가 천장에 부딪히지 않도록 전체 도화지 높이 상한선을 4개 분량(344px)으로 시원하게 오픈합니다!
                  height: 'calc(29.6vw + 24px)', 
                  maxHeight: '344px', 
                  width: '100%', 
                  ...
                  {ACTIVE_LAYOUT.topSpecials[i].map((semiVal, tIdx) => (
                    <div key={tIdx} style={{ width: '100%', height: '7.4vw', maxHeight: '80px', visibility: semiVal === null ? 'hidden' : 'visible', flexShrink: 0 }}>
                      <NoteBox semi={semiVal} getNote={getNoteName} activeNote={activeNote} cents={centsOff} limit={tolerance} onStart={handleNoteStart} onStop={handleNoteStop} isBlowZone={true} holeNum={h} showOverbanding={showOverbanding} selectedTuning={selectedTuning} />
                    </div>
                  ))}
                </div>

                {/* 2. 표준 블로우 행 (ACTIVE_LAYOUT 변환) */}
                <div style={{ width: '100%', height: '7.4vw', maxHeight: '80px', flexShrink: 0, marginBottom: '12px' }}>
                  <NoteBox semi={ACTIVE_LAYOUT.blow[i]} getNote={getNoteName} activeNote={activeNote} cents={centsOff} limit={tolerance} onStart={handleNoteStart} onStop={handleNoteStop} showOverbanding={showOverbanding} />
                </div>
                
                {/* 3. 중앙 가이드 홀 번호 행 (💡 하모니카 중심축으로 스케일 칩 전격 도킹 마스터 패치 완료) */}
                <div style={{ ...BOX_STYLE.holeNumber, margin: '0 0 4px 0', position: 'relative' }}>
                  {h}

                  {/* 💡 [마법의 샌드위치 매립] 가이드 홀 번호판 상단 모서리 선을 절대 축으로 삼아, 블로우 행과 홀 번호 행 사이의 가로 여백 틈새에 정확히 칩을 얹습니다. */}
                     {/* 🎯 [가로 1줄 절대 보증 너비 고정 패치 완결본] 
                      도화지 너비를 고정하여 공간 부족으로 인한 임의 줄바꿈 현상을 원천 박멸하고 완벽한 가로 일직선을 고수합니다. */}
                   {i === 0 && (
                    <div style={{
                      width: '650px', 
                      display: 'flex',
                      flexDirection: 'row',       // 무조건 왼쪽에서 오른쪽 가로 1줄 배치
                      flexWrap: 'nowrap',         // 브라우저의 임의 줄바꿈 물리적 차단
                      alignItems: 'center',       // 수직 정중앙 정렬
                      justifyContent: 'flex-start',
                      gap: '14px',                // 스케일 명칭과 음이름 리스트 사이의 시원한 공백
                      
                      // 🎯 3번째 줄 블로우 박스 아랫선과 4번째 홀 번호판 사이의 가로 경계선 빈 틈새 라인으로 미세 조율하는 절대 수직 좌표축
                      position: 'absolute',
                      top: '-260px',               // 스케일 노트 수직 배열 위치값
                      left: '0px',                // 👈 1번 홀 가장 왼쪽 수직 벽선과 일치
                      zIndex: 300,                // 음정 사각형 글씨 레이어 위로 선명하게 표출되도록 최상위 선점
                      
                      // ❌ [바깥 배경 상자 및 아머 마스크 전면 철거]
                      backgroundColor: 'transparent', // 👈 배경색을 투명하게 완전히 비워 숨김
                      backdropFilter: 'none',         // 유리 질감 이펙트 걷어냄
                      border: 'none',                 // 테두리선 완벽 차단
                      padding: 0,                     // 내부 패딩 여백 원천 제로화
                      boxSizing: 'border-box',
                      pointerEvents: 'none',       
                      boxShadow: 'none',              // 그림자 제거로 투명화 완성
                      overflow: 'hidden'
                    }}>
                      
                      {/* [파트 1]: 박스 없이 순수하게 표출되는 [전조 키 + 스케일 종류] 라벨 텍스트 */}
                      <div style={{ 
                        display: 'flex',
                        flexDirection: 'row',     
                        flexWrap: 'nowrap',
                        alignItems: 'center',
                        gap: '6px',
                        flexShrink: 0             // 공간이 좁아져도 절대 글씨가 쪼그라들지 않게 락
                      }}>
                        {/* 스케일 이름 풀네임 (명품 하늘색 컬러 브랜딩) */}
                        <span style={{ fontSize: '18px', fontWeight: '950', color: '#60a5fa', letterSpacing: '-0.3px', whiteSpace: 'nowrap', textShadow: '0 2px 4px rgba(0,0,0,0.9)' }}>
                          {scaleRootKey} {selectedScale}
                        </span>
                        {/* 가이드 라벨 폰트 (슬림 다크 그레이 고대비 매칭) */}
                        <span style={{ fontSize: '18px', color: '#475569', fontWeight: '900', letterSpacing: '0.5px', whiteSpace: 'nowrap' }}>
                          SCALE NOTES
                        </span>
                      </div>
                      
                      {/* [파트 2]: 박스 없이 순수하게 한 줄로 흐르는 [화성학 구성음 알파벳] 리스트 */}
                      <div style={{ 
                        display: 'flex', 
                        flexDirection: 'row',     // 음명들도 무조건 가로 일렬 배치
                        flexWrap: 'nowrap',       // 줄바꿈 100% 차단
                        alignItems: 'center', 
                        gap: '14px',              // 💡 검은 상자가 완전히 사라졌으므로 알파벳끼리 겹치지 않게 시원한 수평 여백 확보
                        flex: 1,                  // 남은 가로 공간을 가득 채우도록 확장
                        overflowX: 'hidden'
                      }}>
                        {scaleNotesResult.map((note, idx) => (
                          <span 
                            key={idx} 
                            style={{
                              // 🎯 [기존 검은 사각형 div 박스를 완벽히 철거하고 span 단독 초고해상도 폰트 기법 적용]
                              fontSize: '22px',         // 22px 초거대 볼드 알파벳 텍스트 100% 고수
                              fontWeight: '950',        // 울트라 볼드 중후한 폰트 두께감 형성
                              // 근음(첫 음과 마지막 옥타브 음)은 파란색, 나머지 스케일 구성음은 단정한 화이트 매칭
                              color: idx === 0 || idx === scaleNotesResult.length - 1 ? '#60a5fa' : '#cbd5e1',
                              textShadow: '0 2px 5px rgba(0,0,0,1)', // 💡 뒷배경 음정 사각형과 겹쳐도 글씨가 선명하게 뚫고 보이도록 강력한 그림자 후광 주입
                              userSelect: 'none',
                              whiteSpace: 'nowrap',     // 글자가 쪼개지는 현상 방지
                              letterSpacing: '-0.5px',
                              flexShrink: 0             // 음명 알파벳들이 찌러들지 않도록 철통 가드
                            }}
                          >
                            {note}
                          </span>
                ...
                {/* 4. 표준 드로우 행 (ACTIVE_LAYOUT 변환) */}
                <div style={{ width: '100%', height: '7.4vw', maxHeight: '80px', flexShrink: 0, marginBottom: '10px' }}>
                  <NoteBox semi={ACTIVE_LAYOUT.draw[i]} getNote={getNoteName} activeNote={activeNote} cents={centsOff} limit={tolerance} onStart={handleNoteStart} onStop={handleNoteStop} showOverbanding={showOverbanding} />
                </div>

                {/* 5. 하단 벤딩 존 스택 구역 (ACTIVE_LAYOUT 변환) */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', height: 'calc(22.2vw + 16px)', maxHeight: '256px', width: '100%', justifyContent: 'start', alignItems: 'center' }}>
                ...
                {h === 10 && (
                  <div style={{ position: 'absolute', bottom: '-30px', right: '0px', width: '60vw', maxWidth: '650px', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', pointerEvents: 'none', zIndex: 10, fontFamily: 'sans-serif', lineHeight: '1.3' }}>
                    {/* 현재 어떤 튜닝 모드인지 나타내는 텍스트 라벨 추가 */}
                    <div style={{ fontSize: 'calc(12px + 0.5vw)', minFontSize: '14px', fontWeight: 'bold', color: '#64748b', marginBottom: '2px' }}>
                      [ MODE : {selectedTuning.toUpperCase()} TUNING ]
                    </div>
             ...
             {/* 오디오 대시보드 구역 */}
        {/* 🛠️ [하단부 오디오 대시보드 - 마이크 볼륨 오타 및 하울링 0% 차단 정밀 패치 완결본] */}
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%', gap: '14px', boxSizing: 'border-box', marginTop: '20px' }}>
          
                    {/* 🎼 [1층 축: 배킹트랙 및 속도/피치 통합 3등분 라인 - 중복 스타일 키 완벽 박멸 버전] */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', width: '100%', gap: '14px', alignItems: 'center', boxSizing: 'border-box' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', backgroundColor: '#111827', padding: '8px 14px', borderRadius: '12px', border: '1px solid #1e293b', height: '48px', boxSizing: 'border-box' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', minWidth: 0, flexShrink: 0 }}>
                <Upload size={18} style={{ color: '#60a5fa', flexShrink: 0 }} />
                <BackingTrackPlayer onFileLoaded={handleBackingTrackLoaded} />
              </div>
              <button onClick={toggleTrack} style={{ ...DASHBOARD_STYLE.playBtn, height: '32px', width: '32px', padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                {isPlaying ? <Pause size={14} /> : <Play size={14} />}
              </button>
              <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '2px' }}>
                <span style={{ ...DASHBOARD_STYLE.label, fontSize: '11px' }}>MR VOL ({Math.round(mrVolume * 100)}%)</span>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#111827', padding: '6px 14px', borderRadius: '12px', border: '1px solid #1e293b', height: '48px', boxSizing: 'border-box' }}>
              <span style={{ fontSize: '13px', fontWeight: '900', color: '#94a3b8', whiteSpace: 'nowrap' }}>SPEED</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                {/* 💡 [테두리 완치] border: 'none' 속성을 정확히 명시하여 테두리 선을 깨끗이 지웠습니다. */}
                <button 
                  onClick={() => setPlaybackRate(prev => Math.max(0.4, parseFloat((prev - 0.05).toFixed(2))))}
                  style={{ width: '30px', height: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#374151', color: '#ffffff', border: 'none', borderRadius: '6px', fontSize: '16px', fontWeight: '900', cursor: 'pointer', userSelect: 'none' }}
                >
                  -
                </button>
                <span style={{ fontSize: '14px', fontWeight: '900', color: '#60a5fa', minWidth: '48px', textAlign: 'center' }}>
                  {playbackRate.toFixed(2)}x
                </span>
                <button 
                  onClick={() => setPlaybackRate(prev => Math.min(1.0, parseFloat((prev + 0.05).toFixed(2))))}
                  style={{ width: '30px', height: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#374151', color: '#ffffff', border: 'none', borderRadius: '6px', fontSize: '16px', fontWeight: '900', cursor: 'pointer', userSelect: 'none' }}
                >
                  +
                </button>
            ...
            {/* 1-3. PITCH 증감 버튼 컨트롤러 */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#111827', padding: '6px 14px', borderRadius: '12px', border: '1px solid #1e293b', height: '48px', boxSizing: 'border-box' }}>
              <span style={{ fontSize: '13px', fontWeight: '900', color: '#94a3b8', whiteSpace: 'nowrap' }}>PITCH</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                {/* 💡 [테두리 완치] border: 'none' 속성을 정확히 명시하여 테두리 선을 깨끗이 지웠습니다. */}
                <button 
                  onClick={() => setPitchKeyOffset(prev => Math.max(-6, prev - 1))}
                  style={{ width: '30px', height: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#374151', color: '#ffffff', border: 'none', borderRadius: '6px', fontSize: '16px', fontWeight: '900', cursor: 'pointer', userSelect: 'none' }}
                >
                  -
                </button>
                <span style={{ fontSize: '14px', fontWeight: '900', color: pitchKeyOffset === 0 ? '#10b981' : (pitchKeyOffset > 0 ? '#fbbf24' : '#fb7171'), minWidth: '36px', textAlign: 'center' }}>
                  {pitchKeyOffset > 0 ? `+${pitchKeyOffset}` : pitchKeyOffset}
                </span>
                <button 
                  onClick={() => setPitchKeyOffset(prev => Math.min(6, prev + 1))}
                  style={{ width: '30px', height: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#374151', color: '#ffffff', border: 'none', borderRadius: '6px', fontSize: '16px', fontWeight: '900', cursor: 'pointer', userSelect: 'none' }}
                >
                  +
                </button>
              ...
              {/* 🔴 [2층 축: 배킹트랙 아래 줄 - 믹스&레코드 통합 3등분 라인 - 중복 border 키 완전 완치 패치] */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', width: '100%', gap: '14px', alignItems: 'center', boxSizing: 'border-box' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', height: '48px', boxSizing: 'border-box' }}>
              <button 
                ...
                style={{
                  flex: 1, height: '100%', borderRadius: '10px', backgroundColor: isRecording ? '#ef4444' : '#1e293b', color: 'white',
                  fontWeight: '900', fontSize: '11px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', border: '1px solid #374151'
                }}
              >
              ...
              <button 
                ...
                style={{
                  flex: 1.2, height: '100%', borderRadius: '10px', backgroundColor: isRecordedPlaying ? '#10b981' : '#111827', color: 'white',
                  fontWeight: '900', fontSize: '11px', cursor: recordedUrl ? 'pointer' : 'not-allowed', opacity: recordedUrl ? 1 : 0.4, border: '1px solid #1e293b'
                }}
              >
              ...
              <button 
                onClick={() => {
                  ...
                  a.href = recordedUrl;
                  a.download = `harmonica_mix_${new Date().toISOString().slice(0,10)}.wav`;
                  a.click();
                }}
                disabled={!recordedUrl}
                style={{
                  flex: 1, height: '100%', borderRadius: '10px', 
                  backgroundColor: '#4b5563', color: 'white',
                  fontWeight: '900', fontSize: '11px', cursor: recordedUrl ? 'pointer' : 'not-allowed', opacity: recordedUrl ? 1 : 0.3, border: '1px solid #4b5563'
                }}
              >
                SAVE
              </button>
            ...
            {/* 2-2. 마이크 볼륨 제어 슬라이더 박스 (💡 중복 border 선언문을 단일화 정합 완료했습니다) */}
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '2px', minWidth: 0 }}>
                ...
                <input 
                  ...
                  onChange={(e) => {
                    ...
                    setMicVolume(newVol);
                    if (isRecording && micGain.current) {
                      micGain.current.gain.setValueAtTime(newVol, Tone.getContext().currentTime);
                    }
                  }} 
            ...
            {/* 2-3. 신스 볼륨 제어 슬라이더 박스 (💡 중복 border 선언문을 단일화 정합 완료했습니다) */}
            <div style={{ display: 'flex', alignItems: 'center', backgroundColor: '#111827', padding: '8px 14px', borderRadius: '12px', border: '1px solid #1e293b', height: '48px', boxSizing: 'border-box' }}>
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '2px', minWidth: 0 }}>
        ...
        {/* 설정 모달 */}
        {/* 🛠️ [셋팅창 내부 4대 마스터 셀렉터 균등 크기 및 가로 스택 일괄 정렬 패치] */}
        {showSettings && (
          <div style={MODAL_STYLE.modalOverlay}>
            <div style={MODAL_STYLE.modalContent}>
              ...
              {/* 1. Select Tuning 구역 (4대 마스터 규격 일치 완료) */}
              <div style={{ marginBottom: '20px' }}>
                <span style={{ fontSize: '16px', color: '#94a3b8', fontWeight: 'bold', display: 'block', marginBottom: '8px' }}>Harp Tuning 하프 튜닝</span>
                  ...
                  style={{ width: '100%', background: '#1e293b', color: '#60a5fa', border: '1px solid #374151', borderRadius: '12px', padding: '12px', fontSize: '16px', fontWeight: '900', outline: 'none' }}
                >
                  {['Richter', 'Country', 'Melody Maker', 'Natural Minor', 'Harmonic', 'Paddy Richter'].map(t => (
                    <option key={t} value={t}>
                      {t === 'Harmonic' ? 'Harmonic Minor Tuning' : (t === 'Paddy Richter' ? 'Paddy Richter Tuning' : `${t} Tuning`)}
                    </option>
                  ))}
                </select>
              </div>

              {/* 2. Select Key & 3. Select Scale 구역 (가로 1:1 하프 그리드 정렬) */}
              ...
              {/* 4. 💡 [전면 개조] Select Low Key Harp 구역 (위의 박스들과 가로폭/패딩/폰트 크기 100% 동일 규격화) */}
              <div style={{ marginBottom: '22px' }}>
                ...
                <select 
                  ...
                  style={{ width: '100%', background: '#1e293b', color: isLowKey ? '#10b981' : '#ffffff', border: '1px solid #374151', borderRadius: '12px', padding: '12px', fontSize: '16px', fontWeight: '900', outline: 'none' }}
                >
                  <option value="OFF" style={{ color: '#ffffff', fontWeight: 'bold' }}>Standard Harp (OFF)</option>
                  <option value="ON" style={{ color: '#10b981', fontWeight: 'bold' }}>Low Key Harp (ON)</option>
                </select>
              </div>
              ...
              {/* 7. 공간계 조절 구역 (리버브 3단 버튼 인라인 정렬 완료) */}
              <div style={{ marginBottom: '25px', padding: '18px', backgroundColor: '#1f2937', borderRadius: '14px', border: '1px solid #374151' }}>
                ...
                <div style={{ borderTop: '1px solid #374151', paddingTop: '14px', marginTop: '10px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <span style={{ fontSize: '15px', color: '#94a3b8', fontWeight: 'bold' }}>Reverb Amount 리버브 효과량(Wet Level)</span>
...
function NoteBox({ semi, getNote, activeNote, cents, limit, onStart, onStop, isBlowZone, isDrawZone, isTopBb, holeNum, showOverbanding, selectedTuning }) { // 👈 selectedTuning 속성을 받아옵니다.
  ...
  if (isActive) { 
    ...
    if (isTopBb) bgColor = '#93c5fd'; 
    else if (isBlowZone) {
      // 💡 [정밀 색상 보정 패치] 
      // 내추럴 마이너 혹은 하모닉 마이너 모드이면서 7번 홀일 때, 그리고 8~10번 홀일 때는 완벽한 하늘색(#93c5fd)으로 매핑합니다.
      if ((selectedTuning === 'Natural Minor' || selectedTuning === 'Harmonic') && holeNum === 7) {
        bgColor = '#93c5fd'; // 7번홀 오버블로우 B자리 하늘색 전격 복원
      } else if (holeNum >= 8 && holeNum <= 10) {
        bgColor = '#93c5fd'; // 8~10번홀 기본 하늘색
      } else {
        bgColor = '#fca5a5'; // 나머지 상단 스택 연분홍(빨간색) 계열 유지
      }
    }
