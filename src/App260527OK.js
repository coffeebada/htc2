...
const MODAL_STYLE = {
  ...
  modalContent: { 
    ...
    width: '600px',           // 👈 기존 480px에서 600px로 셋팅창 너비를 웅장하게 확장
    borderRadius: '28px', 
    padding: '40px',          // 내부 여백을 넓혀 텍스트 배치가 답답하지 않게 조율
...
function App() {
  ...
  const [selectedScale, setSelectedScale] = useState('Ionian / Major');
    // 💡 [추가] 셋팅 메뉴 내 정밀 튜너 모드 가동을 위한 상태 스위치
  const [usePrecisionTuner, setUsePrecisionTuner] = useState(false);
      ...
      <div style={BOX_STYLE.contentWrapper}>
       ...
       <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', flexShrink: 0 }}>
            <span style={{ fontSize: '14px', color: '#64748b', fontWeight: '900', letterSpacing: '0.5px' }}>SCALE NOTES</span>
            <span style={{ fontSize: '20px', fontWeight: '900', color: '#60a5fa', whiteSpace: 'nowrap' }}>
              {scaleRootKey} {selectedScale.split(' ')[0]} 
            </span>
          </div>
          
          {/* 가로 텍스트 음 배열 구역 */}
          <div style={{ display: 'flex', gap: '14px', flexWrap: 'nowrap', overflowX: 'hidden', alignItems: 'center' }}>
            {scaleNotesResult.map((note, idx) => (
              <span 
                key={idx} 
                style={{
                  // 💡 기존 13px에서 22px 초거대 폰트로 확대하여 시인성 극대화
                  fontSize: '22px', 
                  fontWeight: '900',
                  // 근음(첫 음과 마지막 옥타브 음)은 더 밝은 파란색으로 차별화하여 강조
          ...
          <div style={MODAL_STYLE.modalOverlay}>
            <div style={MODAL_STYLE.modalContent}>
              ...
              <div style={{ marginBottom: '22px', padding: '18px', backgroundColor: '#1f2937', borderRadius: '14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #374151' }}>
                <span style={{ fontSize: '18px', color: '#cbd5e1', fontWeight: 'bold' }}>Harmonica Low Key</span>
              ...
              <div style={{ marginBottom: '22px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                ...
                <div>
                  <span style={{ fontSize: '16px', color: '#94a3b8', fontWeight: 'bold', display: 'block', marginBottom: '8px' }}>Select Scale</span>
                  <select 
                    value={selectedScale} 
                  ...
                  >
                    {[
                      ...
                      'Aeolian / Natural Minor', 'Locrian', 'Major Pentatonic', 
              ...
              <div style={{ marginBottom: '22px' }}>
                <span style={{ fontSize: '16px', color: '#94a3b8', fontWeight: 'bold', display: 'block', marginBottom: '6px' }}>Standard Pitch: A={baseFreq}Hz</span>
              ...
              {/* 4. 허용 오차 디테일 구역 */}
              <div style={{ marginBottom: '22px' }}>
                <span style={{ fontSize: '16px', color: '#94a3b8', fontWeight: 'bold', display: 'block', marginBottom: '8px' }}>Tolerance (±{tolerance}c)</span>
              ...
              {/* 5. 공간계 통합 조절 구역 (리버브 양 슬라이더 전격 탑재) */}
              <div style={{ marginBottom: '25px', padding: '18px', backgroundColor: '#1f2937', borderRadius: '14px', border: '1px solid #374151' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                  <span style={{ fontSize: '16px', color: '#94a3b8', fontWeight: 'bold' }}>Reverb Type</span>
                  <div>
                    {['standard', 'spring'].map(m => (
                      <button key={m} onClick={() => setReverbMode(m)} style={{ padding: '8px 14px', marginLeft: '8px', backgroundColor: m === reverbMode ? '#10b981' : '#374151', border: 'none', color: m === reverbMode ? 'black' : 'white', fontWeight: 'bold', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>
                        {m.toUpperCase()}
                      </button>
                    ))}
                  </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                  <span style={{ fontSize: '18px', color: '#cbd5e1', fontWeight: 'bold' }}>Reverb Power</span>
                  <button onClick={() => setUseReverb(!useReverb)} style={{ padding: '10px 22px', backgroundColor: useReverb ? '#10b981' : '#ef4444', border: 'none', color: useReverb ? 'black' : 'white', borderRadius: '10px', fontWeight: '900', fontSize: '15px', cursor: 'pointer' }}>
                    {useReverb ? 'ON' : 'OFF'}
                  </button>
                </div>

                {/* 💡 [신규 추가] 실시간 공간 깊이 조절 리버브 WET 슬라이더 모듈 */}
                <div style={{ borderTop: '1px solid #374151', paddingTop: '14px', marginTop: '10px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <span style={{ fontSize: '15px', color: '#94a3b8', fontWeight: 'bold' }}>Reverb Amount (Wet Level)</span>
                  ...
                  <input 
                    ...
                    disabled={!useReverb} // 리버브 전원이 꺼지면 슬라이더 비활성화 가드
