...
// =========================================================================
// 🎯 [교정 완결] 전통 복음 하모니카 교본 규격의 옥타브 도트(점) 마크 위치 연산기 (7번홀 컷 적용)
// =========================================================================
function getTremoloTopRowDots(holeNum) {
  // 1번 홀 아래 점 2개
  if (holeNum === 1) return { position: 'bottom', count: 2 };
  // 2~7번 홀 아래 점 1개 (1행은 지시하신 대로 7번홀 한계선 가드 작동)
  if (holeNum >= 2 && holeNum <= 7) return { position: 'bottom', count: 1 };
  // 21번, 23번 홀 위 점 2개 (가로 정렬 체인)
  if (holeNum === 21 || holeNum === 23) return { position: 'top', count: 2 };
  // 15, 17, 18, 19, 20, 22, 24번 홀 위 점 1개
  if ([15, 17, 18, 19, 20, 22, 24].includes(holeNum)) return { position: 'top', count: 1 };
  
  return { position: 'none', count: 0 };
}
// =========================================================================
// 🎯 [교정 완결] 트레몰로 2번째 줄(하단 행) 전용 독립 옥타브 도트 연산기 (8번홀 & 16번홀 포함)
// =========================================================================
function getTremoloBottomRowDots(holeNum) {
  // 1번 홀 아래 점 2개
  if (holeNum === 1) return { position: 'bottom', count: 2 };
  // 2~8번 홀 아래 점 1개 (2행은 지시하신 대로 8번홀 한계선 가드 작동)
  if (holeNum >= 2 && holeNum <= 8) return { position: 'bottom', count: 1 };
  // 21번, 23번 홀 위 점 2개 (가로 정렬 체인)
  if (holeNum === 21 || holeNum === 23) return { position: 'top', count: 2 };
  // 15 ~ 20번 홀 전체 연속 구간 및 22, 24번 홀 정밀 매핑 (유령 오타 전면 소각 완결)
  if ((holeNum >= 15 && holeNum <= 20) || holeNum === 22 || holeNum === 24) {
    return { position: 'top', count: 1 };
  }
      ...
      <div style={BOX_STYLE.contentWrapper}>
        ...
        {isTremolo && (
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '1vh', boxSizing: 'border-box' }}>
            ...
            {/* 🎯 [화면 크기 맞춤 자동 축소 가드] 24홀이 무조건 한눈에 들어오는 가변 그리드 기판 */}
            <div style={{ 
              display: 'flex', 
              flexDirection: 'row',
              flexWrap: 'nowrap', 
              ...
              {Array.from({ length: 24 }).map((_, i) => {
                ...
                // 🎯 1행과 2행 각각의 독립 옥타브 도트 조건 데이터 추출
                const topDots = getTremoloTopRowDots(h);
                const bottomDots = getTremoloBottomRowDots(h);

                // 🎯 [순백색 #ffffff 가로 정렬] 박스 공간에 영향 주지 않는 absolute 도트 생성기
                ...
                return (
                  <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative', flex: '1 1 0%', minWidth: '0px', boxSizing: 'border-box' }}>
                    ...
                    {/* 🎯 행 1: [첫번째줄 부하 하프 상단 레이어] - 1행 전용 흰색 도트 표식 정상 구동 */}
                    ...
                    {/* 🎯 행 2: [두번째줄 온음 C키 하단 레이어] - 2층 흰색 점막 완벽 적용 */}
                    ...
                    {/* 행 3: [중앙 순수 가이드 홀 번호 행] - 1367번 줄 오타 완벽 수리 마감 */}
